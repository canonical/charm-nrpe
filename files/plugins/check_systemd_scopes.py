...
def check_systemd_scopes(args):
    ...
    if error_count >= args.crit_error:
        ...
        raise WarnError(
            'WARNING: System has {} systemd scopes in error state'
            .format(error_count)
        )
    ...
    if error_count >= args.crit_abandoned:
        ...
        raise WarnError(
            'WARNING: System has {} systemd scopes in abandoned state'
            .format(error_count)
        )
    ...
    print("OK: {}; {} in error state, {} in abandoned state".format(
...
def parse_args(args):
    ...
    parser.add_argument(
        "-e", "--warn-error", type=positive_int,
        default=DEFAULT_WARN_ERROR, metavar="WARN_THRESH",
        help="At the specified threshold number of error state scopes, "
        "raise a nagios WARN state"
    )
    parser.add_argument(
        "-E", "--crit-error", type=positive_int,
        default=DEFAULT_CRIT_ERROR, metavar="CRIT_THRESH",
        help="At the specified threshold number of error state scopes, "
    ...
    parser.add_argument(
        "-a", "--warn-abandoned", type=positive_int,
        default=DEFAULT_WARN_ABANDONED, metavar="WARN_THRESH",
        help="At the specified threshold number of abandoned state scopes, "
        "raise a nagios WARN state"
    )
    parser.add_argument(
        "-A", "--crit-abandoned", type=positive_int,
        default=DEFAULT_CRIT_ABANDONED, metavar="CRIT_THRESH",
        help="At the specified threshold number of abandoned state scopes, "
        "raise a nagios CRIT state"
    )
    ...
    parser.add_argument(
        "-o", "--ok-message", default="Nominal", metavar="MSG",
