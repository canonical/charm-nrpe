...
class TestNrpe(TestBase):
    ...
    def test_03_user_monitor(self):
        ...
        local_nrpe_checks = {
            "check_proc_jujud_user.cfg":
                "command[check_proc_jujud_user]=/usr/lib/nagios/plugins/check_procs -w 1 -c 1 -C jujud",
            "check_proc_rsync_user.cfg":
                "command[check_proc_rsync_user]=/usr/lib/nagios/plugins/check_procs -w 1 -c 1 -C rsync"
        }
    ...
    def test_05_netlinks(self):
        ...
        model.set_application_config(
            self.application_name, {"netlinks": netlinks}
        )
        model.block_until_all_units_idle()
        cmd = "cat /etc/nagios/nrpe.d/check_netlinks_eth0.cfg"
        line = "command[check_netlinks_eth0]=/usr/local/lib/nagios/plugins/check_netlinks.py -i eth0 -m 9000 -s 1000"
        result = model.run_on_unit(self.lead_unit_name, cmd)
        code = result.get("Code")
        if code != "0":
            raise model.CommandRunFailed(cmd, result)
